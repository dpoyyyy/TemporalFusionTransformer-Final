...
Original file is located at
    https://colab.research.google.com/drive/10RBITmxv9l4NpkV0xmf0IcHksB5yxwls
"""
...
# ======================================================================================
#  بخش ۱: توابع کمکی (Sparsemax, GLU)
# ======================================================================================

def sparsemax(input: torch.Tensor, dim: int = -1) -> torch.Tensor:
    """پیاده‌سازی کامل و ایمن تابع Sparsemax."""
    if input.dim() == 0:
        return input
...
# ======================================================================================
#  بخش ۲: ماژول‌های اصلی TFT
# ======================================================================================

class GRN(nn.Module):
    """Gated Residual Network - نسخه نهایی و وفادار به مقاله با Dropout."""
...
class StaticCovariateEncoder(nn.Module):
    """انکودر متغیرهای ایستا (SCE)."""
    def __init__(self, d_model: int, dropout_rate: float = 0.1):
        super().__init__()
        self.context_encoders = nn.ModuleDict({n: GRN(d_model, d_model, dropout_rate=dropout_rate) for n in ['c_s', 'c_c', 'c_h', 'c_e']})
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        return {n: enc(x) for n, enc in self.context_encoders.items()}

class Seq2SeqLSTM(nn.Module):
    """لایه پردازش محلی با LSTM به همراه Dropout."""
...
class InterpretableMultiHeadAttention(nn.Module):
    """مکانیزم توجه چند-سر قابل تفسیر."""
...
class TemporalProcessor(nn.Module):
    ...
    def forward(self, hist_emb: torch.Tensor, fut_emb: torch.Tensor, contexts: Dict[str, torch.Tensor], causal_mask: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        residual_for_final_gate = enriched
        ...
        final_out = self.final_gate(residual_for_final_gate, c=ffn_out)
        return final_out, alpha

class TemporalFusionTransformer(nn.Module):
    """مدل کامل و یکپارچه Temporal Fusion Transformer."""
    ...
    def forward(self, static_vars: Dict[str, torch.Tensor], past_vars: Dict[str, torch.Tensor], future_vars: Dict[str, torch.Tensor], return_interpretation: bool = False):
        ...
        if return_interpretation:
            interpretation = {"past_vsn_weights": past_weights, "future_vsn_weights": future_weights, "attention_weights": attn_weights, "static_vsn_weights": static_weights}
